#!/usr/bin/env python3
"""
🔥 ULTRA-FAST BULK UPLOAD PERFORMANCE TEST 🔥

Tests the new ultra-fast bulk upload API with:
1. In-memory batch processing
2. Bulk employee ID generation
3. Single batch database insert
4. Postfix collision handling (SID-MA-025-A, B, C...)

This should be SIGNIFICANTLY faster than the old method!
"""
...
sys.path.append('/mnt/c/Users/sddha/Coding/Sniper/Final HRms/hrms v2/New hrms/Hrms/Hrms/backend-tally-dashboard')
...
def test_bulk_upload_performance(num_employees=100):
    ...
    # Add headers for tenant (if needed)
    headers = {
        'X-Tenant-ID': '1',  # Adjust based on your tenant setup
    }
    ...
    try:
        ...
        print(f"\n⏰ TIMEOUT! Upload took longer than 5 minutes")
...
def test_collision_handling():
    ...
    try:
        response = requests.post(BULK_UPLOAD_URL, files=files, headers=headers)
...
def run_performance_comparison():
    """Run multiple tests with different data sizes"""
    print(f"\n🏁 ULTRA-FAST BULK UPLOAD PERFORMANCE COMPARISON")
    print("=" * 60)
    
    test_sizes = [10, 50, 100, 200, 500]  # Start with smaller sizes
    results = {}
    
    for size in test_sizes:
        print(f"\n{'='*20} TESTING {size} EMPLOYEES {'='*20}")
        upload_time = test_bulk_upload_performance(size)
        if upload_time:
            results[size] = upload_time
            rate = size / upload_time
            print(f"🎯 RESULT: {size} employees in {upload_time:.2f}s ({rate:.1f} emp/sec)")
    ...
    print(f"\n📊 PERFORMANCE SUMMARY:")
    print("=" * 40)
    for size, upload_time in results.items():
        rate = size / upload_time
        print(f"   {size:3d} employees: {upload_time:6.2f}s ({rate:5.1f} emp/sec)")
    
    if results:
        avg_rate = sum(size / time for size, time in results.items()) / len(results)
        print(f"\n🚀 AVERAGE RATE: {avg_rate:.1f} employees/second")
...
if __name__ == "__main__":
    ...
    try:
        ...
        test_bulk_upload_performance(100)
        
        # Test 2: Collision handling
        test_collision_handling()
        
        # Test 3: Performance comparison
        # run_performance_comparison()  # Uncomment for full benchmark
